...
object Seven {
  val t: List[Try[Int]] = List()
  def main(args: Array[String]): Unit = {

    println(Try.apply("123".toInt)) // Will succeed and print Success(123)
    ...
    val strs = List("123", "234", "-123", "aa") // An acceptable list of integers
    println(strs.map(s => Try(s.toInt)))
    val lt = strs.map(s => Try(s.toInt))
    ...
    val r = Try {
      println(
        "Executed in Try apply method because this parameter is pass by name"
      )
      "1234".toInt
    }
  }
